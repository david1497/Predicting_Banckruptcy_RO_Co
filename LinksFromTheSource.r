...
ListOfUrls = data.frame(link=character())
...
for (i in 1:z) {
  ...
  ListOfUrls <- rbind(ListOfUrls, data.frame(n <- a))
}

ListOfCompanyUrls = data.frame(link=character())
#ListOfCompanyUrls1 = data.frame(link=character())
#ListOfCompanyUrls = c()
...
l <- vector("list")
for (i in 2830:z) {
  urlOfTheList = ListOfUrls$n[i]
  theLinks <- urlOfTheList %>%
    read_html() %>%
    html_nodes(".clickable-row a") %>%
    html_attr("href")
  for (x in 1:length(theLinks)){
    if(is.element(theLinks[x], a$matrix.unlist.l...nrow...length.l...byrow...TRUE.)){
      print("Shit in the box!")
    } else{
      l[[length(l)+1]] <- theLinks[x]  
    }
  }
  
  if(length(ListOfCompanyUrls$link) < 1){
    print("Nothing to put in, shit avoided!")
  } else {
    ListOfCompanyUrls <- data.frame(matrix(unlist(l), nrow=length(l), byrow=TRUE), stringsAsFactors=FALSE)
  }
  if (i%%10 == 0){
    print(paste0("Page ", i, " -- ", (i*50)/(z*50)*100, " % copied"))
    write.csv(ListOfCompanyUrls, file = "newfile2.csv")
  }
}
